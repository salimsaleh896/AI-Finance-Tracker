...
const App = () => {
  ...
  // NEW: Logic to filter transactions based on selected timeframe
  ...
  // Updated chartData to use filteredTransactions
        ...
        <div className="flex items-center gap-6">
          <div className="p-4 bg-slate-900 rounded-xl border border-slate-800 text-right shadow-lg">
            ...
            <p className="text-2xl font-mono text-white">
              ...
              Ksh{filteredTransactions.reduce((acc, curr) => acc + curr.amount, 0).toFixed(2)}
            </p>
          </div>
            ...
            {filteredTransactions.length > 0 ? filteredTransactions.map((t) => (
              <div key={t._id} className="flex justify-between items-center bg-slate-950 p-4 rounded-xl border border-slate-900 group hover:border-slate-700 transition-all">
                ...
                <div className="flex items-center gap-4">
                  <p className="text-red-400 font-mono font-bold">-Ksh{Number(t.amount).toFixed(2)}</p>
                  <button
                    onClick={() => deleteTransaction(t._id)}
                    className="text-slate-700 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100 text-xl font-bold"
                  >
                    ×
                  </button>
                </div>
