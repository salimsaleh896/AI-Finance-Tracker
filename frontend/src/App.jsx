...
const App = () => {
  ...
  const fetchTransactions = async () => {
    ...
    try {
      const res = await axios.get(`${API_URL}/api/transactions`, {
        headers: { Authorization: token } // Sending raw token
      });
  ...
  return (
    <div className="min-h-screen p-8 max-w-6xl mx-auto bg-slate-950 text-white">
        ...
        <section className="bg-slate-900 p-6 rounded-2xl border border-slate-800">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <input
              ...
              required
            />
            <button disabled={loading} className="w-full bg-blue-600 hover:bg-blue-500 py-3 rounded-lg font-bold transition-all disabled:opacity-50">
        ...
        <section className="bg-slate-900 p-6 rounded-2xl border border-slate-800 max-h-[500px] overflow-y-auto">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex gap-1 bg-slate-950 p-1 rounded-lg">
              {['all', 'week', 'month'].map((f) => (
                <button key={f} onClick={() => setFilter(f)} className={`px-2 py-1 text-[10px] rounded uppercase font-bold ${filter === f ? 'bg-blue-600' : 'text-slate-500'}`}>
                  {f}
                </button>
              ))}
            </div>
          ...
          <div className="space-y-3">
            {filteredTransactions.map((t) => (
              <div key={t._id} className="flex justify-between items-center bg-slate-950 p-4 rounded-xl border border-slate-900 group">
                <div>
                  <p className="font-medium">{t.title}</p>
                  <span className="text-[10px] bg-slate-800 px-2 py-0.5 rounded text-blue-300 uppercase font-bold">{t.category}</span>
                </div>
                <div className="flex items-center gap-4">
                  <p className="text-red-400 font-mono font-bold">-Ksh {Number(t.amount).toLocaleString()}</p>
                  <button onClick={() => deleteTransaction(t._id)} className="text-slate-700 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100 font-bold">×</button>
                </div>
              </div>
            ))}
          </div>
